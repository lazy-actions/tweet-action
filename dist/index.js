...
/** @type {string} */
var _VERSION_STRING = __nccwpck_require__(499)/* .version */ .i8;
...
const currentDir = process.env.GITHUB_WORKSPACE || '';
function loadFromFile(filename) {
    const filepath = external_path_.isAbsolute(filename)
        ? filename
        : external_path_.join(currentDir, filename);
    if (!external_fs_.existsSync(filepath)) {
        throw new Error(`Could not find ${filename}`);
    }
    if (!external_fs_.statSync(filepath).isFile()) {
        throw new Error(`${filename} is not a file`);
    }
    return external_fs_.readFileSync(filepath, { encoding: 'utf8' });
}
function camel2snake(target) {
    return target.replace(/[A-Z]/g, (match) => `_${match.toLowerCase()}`);
}
function rfc3986(target) {
    return encodeURIComponent(target).replace(/[!'()*]/g, (x) => `%${x.charCodeAt(0).toString(16).toUpperCase()}`);
}
...
var ejs = __nccwpck_require__(431);
...
;// CONCATENATED MODULE: ./src/oauth.ts
...
function tweet(message, oauthConsumerKey, oauthConsumerSecret, oauthToken, oauthTokenSecret) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!res.ok) {
            try {
                ...
                throw new Error(`Failed to post a tweet: ${JSON.stringify(resBody.errors)}`);
            }
            catch (err) {
...
function run() {
    return src_awaiter(this, void 0, void 0, function* () {
        ...
        if (!message) {
            message = render(inputs.template, inputs.data);
            core.debug(`Rendered message: ${message}`);
        }
        yield tweet(message, inputs.oauthConsumerKey, inputs.oauthConsumerSecret, inputs.oauthToken, inputs.oauthTokenSecret);
